...
class CNNLSTMModel(object):
	...
	def __buildModel__(self,
		...
		###############
		l_conv_0_a = lasagne.layers.Conv2DLayer(
			l_transpose, num_filters = 16, filter_size = (7, 7), stride = (1, 1), pad = 'same',
			W = lasagne.init.HeUniform(), nonlinearity=lasagne.nonlinearities.linear, 
			name = "l_conv_0_a")
		l_conv_0_b = lasagne.layers.Conv2DLayer(
			l_transpose, num_filters = 16, filter_size = (5, 5), stride = (1, 1), pad = 'same',
			W = lasagne.init.HeUniform(), nonlinearity=lasagne.nonlinearities.linear, 
			name = "l_conv_0_b")
		l_conv_0_c = lasagne.layers.Conv2DLayer(
			l_transpose, num_filters = 16, filter_size = (3, 3), stride = (1, 1), pad = 'same',
			W = lasagne.init.HeUniform(), nonlinearity=lasagne.nonlinearities.linear, 
			name = "l_conv_0_c")
		l_conv_0_d = lasagne.layers.Conv2DLayer(
			l_transpose, num_filters = 16, filter_size = (1, 1), stride = (1, 1), pad = 'same',
			W = lasagne.init.HeUniform(), nonlinearity=lasagne.nonlinearities.linear, 
			name = "l_conv_0_d")
		l_concat_0 = lasagne.layers.ConcatLayer([l_conv_0_a, l_conv_0_b, l_conv_0_c, l_conv_0_d], axis=1)
		l_max_0 = lasagne.layers.MaxPool2DLayer(
			l_concat_0, pool_size = (2, 2), ignore_border = False, name = "l_max_0"
			)
		l_bn0 = lasagne.layers.BatchNormLayer(l_max_0, name = "l_bn0")
		l_relu_0 = lasagne.layers.NonlinearityLayer(l_bn0, name = "l_relu_0")
		
		###############
		l_conv_1_a = lasagne.layers.Conv2DLayer(
			l_relu_0, num_filters = 32, filter_size = (5, 5), stride = (1, 1), pad = 'same',
			W = lasagne.init.HeUniform(), nonlinearity=lasagne.nonlinearities.linear, 
			name = "l_conv_1_a")
		l_conv_1_b = lasagne.layers.Conv2DLayer(
			l_relu_0, num_filters = 32, filter_size = (3, 3), stride = (1, 1), pad = 'same',
			W = lasagne.init.HeUniform(), nonlinearity=lasagne.nonlinearities.linear, 
			name = "l_conv_1_b")
		l_conv_1_c = lasagne.layers.Conv2DLayer(
			l_relu_0, num_filters = 32, filter_size = (1, 1), stride = (1, 1), pad = 'same',
			W = lasagne.init.HeUniform(), nonlinearity=lasagne.nonlinearities.linear, 
			name = "l_conv_1_c")
		l_concat_1 = lasagne.layers.ConcatLayer([l_conv_1_a, l_conv_1_b, l_conv_1_c], axis=1)
		l_max_1 = lasagne.layers.MaxPool2DLayer(
			l_concat_1, pool_size = (2, 2), ignore_border = False, name = "l_max_1"
			)
		l_bn1 = lasagne.layers.BatchNormLayer(l_max_1, name = "l_bn1")
		l_relu_1 = lasagne.layers.NonlinearityLayer(l_bn1, name = "l_relu_1")
		
		###############
		l_conv_2_a = lasagne.layers.Conv2DLayer(
			l_relu_1, num_filters = 32, filter_size = (5, 5), stride = (1, 1), pad = 'same',
			W = lasagne.init.HeUniform(), nonlinearity=lasagne.nonlinearities.linear, 
			name = "l_conv_2_a")
		l_conv_2_b = lasagne.layers.Conv2DLayer(
			l_relu_1, num_filters = 32, filter_size = (3, 3), stride = (1, 1), pad = 'same',
			W = lasagne.init.HeUniform(), nonlinearity=lasagne.nonlinearities.linear, 
			name = "l_conv_2_b")
		l_conv_2_c = lasagne.layers.Conv2DLayer(
			l_relu_1, num_filters = 32, filter_size = (1, 1), stride = (1, 1), pad = 'same',
			W = lasagne.init.HeUniform(), nonlinearity=lasagne.nonlinearities.linear, 
			name = "l_conv_2_c")
		l_concat_2 = lasagne.layers.ConcatLayer([l_conv_2_a, l_conv_2_b, l_conv_2_c], axis=1)
		l_max_2 = lasagne.layers.MaxPool2DLayer(
			l_concat_2, pool_size = (2, 2), ignore_border = False, name = "l_max_1"
			)
		l_bn2 = lasagne.layers.BatchNormLayer(l_max_2, name = "l_bn2")
		...
		l_forward_1 = lasagne.layers.LSTMLayer(
			...
			nonlinearity=lasagne.nonlinearities.tanh, only_return_final=False, name = 'l_forward_1')
		l_concat_forward = lasagne.layers.ConcatLayer([l_new_input, l_forward_1], axis=2)
		...
		l_forward_2 = lasagne.layers.LSTMLayer(
			...
			cell_init=lasagne.init.HeUniform(),
			hid_init=lasagne.init.HeUniform(),
			nonlinearity=lasagne.nonlinearities.tanh, only_return_final=False, name = 'l_forward_2')
		...
		######################################## Loss
		l_sl_ = lasagne.layers.SliceLayer(l_forward_2, indices=slice(0, None, self.LABEL_SKIP), axis=1)
