...
class CNNLSTMModel(object):
	...
	def __buildModel__(self,
		...
		#l_bn = lasagne.layers.BatchNormLayer(l_in, axes=(1), name = "l_bn")
		...
		######### Freq -> Time
		l_conv_t = lasagne.layers.Conv1DLayer(
		...
		l_max = lasagne.layers.MaxPool1DLayer(l_conv_t, 161)

		l_reshape_2 = lasagne.layers.ReshapeLayer(l_max, (self.BATCH_SIZE, [1], -1), name = "l_reshape")
		l_transpose_2 = lasagne.layers.DimshuffleLayer(l_reshape_2, (0, 2, 1), name = "l_transpose")
		print(l_transpose_2.input_shape)
		print(l_transpose_2.output_shape)
		
		l_bn_t = lasagne.layers.BatchNormLayer(l_transpose_2, axes=(0, 1), name = "l_bn_t")
		print(l_bn_t.input_shape)
		l_relu_t = lasagne.layers.NonlinearityLayer(l_bn_t, nonlinearity=lasagne.nonlinearities.rectify, name = "l_relu_t")
		...
		l_forward = lasagne.layers.LSTMLayer(
			l_relu_t, self.HIDDEN_DIM, mask_input=l_mask, grad_clipping=self.GRAD_CLIP,
	...
	def _loadNpyFileFromLFB(self, file_name):
		ID2Language = ['EN', 'LM']
	...
	def _loadNpyFileFromSignal(self, file_name):
		ID2Language = ['EN', 'LM']
	...
	def _loadValidData(self, min_len = 48000):
...
def main():
	...
	CLModel.__buildModel__(trainData, validData, language_id = LanguageId, learning_rate = 0.01, input_dim = 560, batch_size = 64, hidden_dim = 256, max_length = 48000)
	...
	for p in all_params:
		print(p.shape)
	while True:
		train_cost = CLModel.train(100, 10)
		print(train_cost / 100)
